            ...
            <div className='flex flex-wrap justify-center gap-x-5 gap-y-5 w-full mx-auto "'>
                {
                    todos.map(function (todo, index) {
                        ...
                        return <div key={index} aria-label={`Todo Number : ${index + 1}`} tabIndex={0} className={`tabs  border h-[30vh] py-5 rounded-xl w-[70vw] lg:w-[30vw] ${theme ? "bg-slate-900 text-slate-100" : "bg-white text-slate-800"}  flex items-center justify-between flex-col`}>
                            <h1 tabIndex={0}>{todo.title}</h1>
                            <p tabIndex={0}>{todo.description}</p>
